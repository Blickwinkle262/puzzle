...
def load_game_entry(entry_path: Path) -> GameEntry:
    entry_path = entry_path.resolve()
    if not entry_path.exists():
        raise ConfigError(f"Entry config does not exist: {entry_path}")

    data = _read_json(entry_path)
    project_root = entry_path.parent.parent

    active_level = _resolve_asset_path(_require_str(data, "active_level"), project_root)
    defaults_raw = data.get("defaults_path")
    defaults_path = _resolve_asset_path(str(defaults_raw), project_root) if defaults_raw else None

    if not active_level.exists():
        raise ConfigError(f"Active level does not exist: {active_level}")
    if defaults_path is not None and not defaults_path.exists():
        raise ConfigError(f"Defaults config does not exist: {defaults_path}")
...
def _parse_level_config(data: dict[str, Any], project_root: Path) -> LevelConfig:
    ...
    bg_image_raw = data.get("background_image")
    background_image = _resolve_asset_path(bg_image_raw, project_root) if bg_image_raw else None
    ...
    if not isinstance(rules_raw, dict):
        raise ConfigError("Field rules must be an object")
    rules = RuleConfig(
        snap_strategy=str(rules_raw.get("snap_strategy", "strict")),
        magnet_strength=_optional_float(rules_raw, "magnet_strength", default=0.0),
    )
    ...
    explicit_font = ui_raw.get("font_path")
    font_path = (
        _resolve_asset_path(str(explicit_font), project_root)
        if explicit_font
        else _discover_default_font_path(project_root)
    )
...
def _optional_float(data: dict[str, Any], key: str, default: float) -> float:
    ...
    if not isinstance(value, (int, float)):
        raise ConfigError(f"Field {key} must be numeric")
    return float(value)
...
def _discover_default_font_path(project_root: Path) -> Path | None:
    search_dirs = [
        project_root / "materials" / "source" / "fonts",
        project_root / "materials" / "source" / "font",
    ]
